...
Original file is located at
    https://colab.research.google.com/drive/1UOzwVhofVfVpFkYT5v5LFeExDomwE4qC
"""
...
class LayerwiseCLIP(nn.Module):
    def __init__(self, model_name="openai/clip-vit-base-patch32", k_dim=256, train_text=True):
        super().__init__()
        self.clip = CLIPModel.from_pretrained(model_name)
        self.k_dim = k_dim
        self.num_layers = self.clip.text_model.config.num_hidden_layers
        self.embed_dim = self.clip.projection_dim
        if self.k_dim <= 0 or self.k_dim > self.embed_dim:
            raise ValueError(f"k_dim must be in [1, {self.embed_dim}], got {self.k_dim}")
    ...
    def encode_text_all_layers(self, input_ids, attention_mask):
        ...
        causal_attention_mask = self._build_causal_attention_mask(
            bsz, seq_len, hidden_states.dtype, device
        )
        full_attention_mask = self._build_attention_mask(
...
class ESECLIP(LayerwiseCLIP):
    def __init__(self, model_name="openai/clip-vit-base-patch32", k_dim=256):
...
def build_express_layer_weights(
    num_layers,
    mode="piecewise_shallow",
    shallow_layers=6,
    shallow_boost=3.0,
    shallow_alpha=2.0,
    shallow_power=1.5,
    ...
    elif mode == "piecewise_shallow":
        weights = np.ones(num_layers, dtype=np.float32)
        shallow_layers = max(1, min(int(shallow_layers), num_layers - 1))
        weights[:shallow_layers] = float(shallow_boost)
    elif mode == "smooth_shallow":
        if num_layers == 1:
            weights = np.ones(num_layers, dtype=np.float32)
        else:
            z = np.linspace(0.0, 1.0, num_layers, dtype=np.float32)
            weights = 1.0 + float(shallow_alpha) * np.power(1.0 - z, float(shallow_power))
    else:
        raise ValueError(f"Unknown express_weight_mode: {mode}")

    weights = weights * (num_layers / weights.sum())
    return weights.astype(np.float32)


def compute_express_loss(text_layer_features, image_features, k_dim, logit_scale, layer_weights=None):
    img_k = F.normalize(image_features[:, :k_dim], dim=-1)
    total = 0.0
    num_layers = len(text_layer_features)

    if layer_weights is None:
        layer_weights = build_express_layer_weights(num_layers, mode="log_decay")
    weights_tensor = torch.as_tensor(
        layer_weights, dtype=image_features.dtype, device=image_features.device
    )

    for idx, txt in enumerate(text_layer_features, start=1):
        txt_k = F.normalize(txt[:, :k_dim], dim=-1)
        total += weights_tensor[idx - 1] * bi_contrastive_loss(txt_k, img_k, logit_scale)
...
def compute_total_loss(model, image_features, text_layer_features, express_layer_weights=None):
    if hasattr(model.clip, "logit_scale"):
        logit_scale = model.clip.logit_scale.exp().clamp(max=100)
    else:
        logit_scale = torch.tensor(1.0, device=image_features.device)

    cross_modal_loss = bi_contrastive_loss(
        text_layer_features[-1], image_features, logit_scale
    )
    express_loss = compute_express_loss(
        text_layer_features, image_features, model.k_dim, logit_scale, express_layer_weights
    )
...
@torch.no_grad()
def encode_all_images(model, image_loader, device, use_prefix=False, k_dim=256):
    model.eval()
    all_feats = []
...
@torch.no_grad()
def encode_all_texts_by_layer(model, text_loader, device, use_prefix=False, k_dim=256):
    model.eval()
...
@torch.no_grad()
def evaluate_model_all_layers(model, image_loader, text_loader, caption_to_image_idx, device, use_prefix=False, k_dim=256):
...
def parse_args():
    ...
    parser.add_argument("--data_root", type=str, default="/home/ziyuanmeng/prj_ziyuanmeng/flickr8k/flickr8k")
    parser.add_argument("--image_dir", type=str, default="/home/ziyuanmeng/prj_ziyuanmeng/flickr8k/flickr8k/images")
    parser.add_argument("--caption_file", type=str, default="/home/ziyuanmeng/prj_ziyuanmeng/flickr8k/flickr8k/captions.txt")
    parser.add_argument("--split_dir", type=str, default=None)
    parser.add_argument("--output_dir", type=str, default="/home/ziyuanmeng/prj_ziyuanmeng/outputs/ese_clip_k256_shallow_weight")
    ...
    parser.add_argument(
        "--express_weight_mode",
        type=str,
        default="piecewise_shallow",
        choices=["piecewise_shallow", "smooth_shallow", "log_decay", "uniform"],
    )
    parser.add_argument("--shallow_layers", type=int, default=6)
    parser.add_argument("--shallow_boost", type=float, default=3.0)
    parser.add_argument("--shallow_alpha", type=float, default=2.0)
...
def main():
    ...
    if args.device == "cuda" and not torch.cuda.is_available():
        raise RuntimeError(
            ...
            "Please run inside a GPU allocation and make sure PyTorch is installed with CUDA support."
        )
    device = args.device
    ...
    train_loader, val_image_loader, val_text_loader = build_dataloaders(
        args, processor, tokenizer, train_pairs, val_images, val_captions
    )
    ...
    print("\n========== Raw CLIP: full 512 ==========")
    raw_full_df = evaluate_model_all_layers(
        raw_clip, val_image_loader, val_text_loader, caption_to_image_idx,
        device=device, use_prefix=False, k_dim=args.k_dim
    )
    ...
    raw_prefix_df = evaluate_model_all_layers(
        raw_clip, val_image_loader, val_text_loader, caption_to_image_idx,
        device=device, use_prefix=True, k_dim=args.k_dim
    )
    ...
    express_layer_weights = build_express_layer_weights(
        ese_model.num_layers,
        mode=args.express_weight_mode,
        shallow_layers=args.shallow_layers,
        shallow_boost=args.shallow_boost,
    ...
    express_weight_info = {
        ...
        "shallow_alpha": args.shallow_alpha,
        "shallow_power": args.shallow_power,
        "weights": [float(x) for x in express_layer_weights.tolist()],
    }
    print("[Express weights]", express_weight_info)
    save_json(express_weight_info, os.path.join(args.output_dir, "express_layer_weights.json"))
    ...
    if not args.eval_only:
        ...
        print(f"[Train] trainable params: {num_trainable/1e6:.2f}M")
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for step, batch in enumerate(pbar, start=1):
                ...
                with amp_ctx:
                    image_features, text_layer_features = ese_model(
                        pixel_values=pixel_values,
                        input_ids=input_ids,
                        attention_mask=attention_mask
                    )
                    loss, loss_dict = compute_total_loss(
                        ese_model, image_features, text_layer_features, express_layer_weights
                    )
            ...
            val_prefix_df = evaluate_model_all_layers(
                ese_model, val_image_loader, val_text_loader, caption_to_image_idx,
                device=device, use_prefix=True, k_dim=args.k_dim
            )
            ...
            if val_score > best_score:
                ...
                torch.save({
                    ...
                    "k_dim": args.k_dim,
                    "express_weight_info": express_weight_info,
                }, best_ckpt_path)
    ...
    print("\n========== ESE-CLIP: full 512 ==========")
    ese_full_df = evaluate_model_all_layers(
        ese_model, val_image_loader, val_text_loader, caption_to_image_idx,
        device=device, use_prefix=False, k_dim=args.k_dim
    )
    ...
    ese_prefix_df = evaluate_model_all_layers(
        ese_model, val_image_loader, val_text_loader, caption_to_image_idx,
        device=device, use_prefix=True, k_dim=args.k_dim
    )
    ...
    compare_df = pd.concat(
        [raw_full_df, raw_prefix_df, ese_full_df, ese_prefix_df],
        ignore_index=True
    )[["Model", "EvalMode", "Layer", "I2T_R@1", "I2T_R@5", "T2I_R@1", "T2I_R@5", "MeanR"]]
    compare_df = compare_df.sort_values(["EvalMode", "Model", "Layer"]).reset_index(drop=True)

    prefix_summary_df = pd.concat([raw_prefix_df, ese_prefix_df], ignore_index=True)
    prefix_summary_df = prefix_summary_df[["Model", "EvalMode", "Layer", "I2T_R@1", "I2T_R@5", "T2I_R@1", "T2I_R@5", "MeanR"]]
    prefix_summary_df = prefix_summary_df.sort_values(["Model", "Layer"]).reset_index(drop=True)
