...
def layer_weight(layer_idx_1based, num_layers):
    if layer_idx_1based == num_layers:
        return 1.0
    i = num_layers - layer_idx_1based + 1
    return 1.0 / (1.0 + math.log(i))


def compute_express_loss(text_layer_features, image_features, k_dim, logit_scale):
    ...
    for idx, txt in enumerate(text_layer_features, start=1):
        ...
        w = layer_weight(idx, num_layers)
        total += w * bi_contrastive_loss(txt_k, img_k, logit_scale)
        total_w += w
...
def compute_total_loss(model, image_features, text_layer_features):
    ...
    express_loss = compute_express_loss(
        text_layer_features, image_features, model.k_dim, logit_scale
    )
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for step, batch in enumerate(pbar, start=1):
                ...
                with amp_ctx:
                    ...
                    loss, loss_dict = compute_total_loss(ese_model, image_features, text_layer_features)
